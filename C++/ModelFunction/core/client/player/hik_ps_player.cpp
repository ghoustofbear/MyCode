...
void psstreamdatacb(uint8_t *data, int size, void* userdata, int type, uint64_t pts, uint64_t frameTime)//回调函数获取封装后的PS流并解码显示
{
    HikPsPlayer *player = (HikPsPlayer*)userdata;
...
: Player(listener)
{
    ...
    m_recfile = NULL;
    m_framePts = 0;
    pspacker = createpsinstance(PS_PACK);               //创建PS封装对象
    setpsstreamcallback(pspacker, psstreamdatacb, this);//设置回调函数
}

HikPsPlayer::~HikPsPlayer()
{
	destorypsinstance(pspacker);                        //释放PS封装对象 
}
...
HikPsPlayer::openReverse(HWND wnd, int streamMode, int encode, int poolSize, void *head, int size)
{
    ...
    do
    {
        ...
        if (!PlayM4_ReversePlay(m_playID))//倒放
        {
            break;
        }
