...
void
MD5(unsigned char *in, unsigned char *crypto, int size)
{
    if (!in || !crypto || !(size >= 32))
    {
        return;
    }
    ...
    MD5_CTX md5;
    MD5Init(&md5);
    MD5Update(&md5, in, (unsigned int)strlen((const char *)in));
    MD5Final(&md5, decrypt);

    for (int i = 0; i < 16; ++i)
    {
        snprintf((char *)crypto + i * 2, 3, "%02x", decrypt[i]);
    }
}
