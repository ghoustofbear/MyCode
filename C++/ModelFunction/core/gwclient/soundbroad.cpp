...
SoundBroadcast::SoundBroadcast(SipUA *pSipUa)
{
	m_pSipUa  = pSipUa;
...
bool SoundBroadcast::start(const char *json, int size)
{
	...
	for (int index = 0; index < int(itemlist.size()); ++index)
	{
		...
		string resp;
		char sdp[1024] = { 0 };
		sprintf(sdp, GW_AUDIO_SDP_FORMAT, m_pSipUa->ip().c_str(), m_pSipUa->ip().c_str(), srcport);
	...
	m_hSound = CAP_start_audio(NULL, &encode, 2, cap_audio_ex, this);
	if (NULL == m_hSound)
	{
