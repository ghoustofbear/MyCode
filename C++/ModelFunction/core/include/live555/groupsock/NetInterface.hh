...
class live555Socket: public NetInterface {
public:
	virtual ~live555Socket();
...
protected:
	live555Socket(UsageEnvironment& env, Port port); // virtual base class
...
UsageEnvironment& operator<<(UsageEnvironment& s, const live555Socket& sock);
...
public:
  ...
  live555Socket* Fetch(UsageEnvironment& env, Port port, Boolean& isNew);
  // Creates a new Socket if none already exists
  Boolean Remove(live555Socket const* sock);

protected:
  SocketLookupTable(); // abstract base class
  virtual live555Socket* CreateNew(UsageEnvironment& env, Port port) = 0;
