...
public:
	StreamListener() : m_buffStatus(S_BUFF_NONE){ m_offset = 0; }
    virtual void       onVideoStream(void *stream, uint32_t size, int encode, int type, int w, int h, int fps, uint64_t pts, uint32_t tag) = 0;
    virtual void       onAudioStream(void *stream, uint32_t size, int encode, int sps,  int channels, int bps, uint64_t pts) = 0;
    virtual void       onPrivStream (void *stream, uint32_t size, int encode, uint32_t tag, int type = 'I') = 0;
...
public:
    ...
    virtual void       doVideoStream(void *stream, uint32_t size) = 0;
    virtual void       doAudioStream(void *stream, uint32_t size, int flag=0) = 0;
