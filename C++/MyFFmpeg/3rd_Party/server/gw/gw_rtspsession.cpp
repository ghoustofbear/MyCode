...
#define  GW_RTSP_SDP_FORMAT \
		...
		"a=fmtp:100 profile-level-id=42A01E; packetization-mode=1; sprop-parameter-sets=Z0IAKOk,aM4wpIAA==\r\n" \
		"a=recvonly\r\n" \
...
GWRTSPSession::~GWRTSPSession()
{
    rtspSV->delBuffevListener(m_bev, this);
}

void
GWRTSPSession::onOption(stk::RtspMsg &rtsp)
{
    ...
    m_bev->push((uint8_t *)data.c_str(), data.size());
}

void
GWRTSPSession::onDescribe(stk::RtspMsg &rtsp)
{
    string url = rtsp.GetReqUrl();
    string seq = rtsp.GetCSeq();
    ...
    char sdp[500] = {0};
    snprintf(sdp, sizeof(sdp), GW_RTSP_SDP_FORMAT, m_bev->srcip().c_str(), m_bev->srcip().c_str(), 0);
...
void
GWRTSPSession::onSetup(stk::RtspMsg &rtsp)
{
    string seq = rtsp.GetCSeq();
...
#if 0
void
GWRTSPSession::onPlay(stk::RtspMsg &rtsp)
{
    string url    = rtsp.GetReqUrl();
    string seq    = rtsp.GetCSeq();
...
GWRTSPSession::onPlay(stk::RtspMsg &rtsp)
{
    ...
    else
    {
        ...
        requrl += 1;
		string suburl = string("rtsp:") + requrl;
...
void
GWRTSPSession::onPause(stk::RtspMsg &rtsp)
{
    string url    = rtsp.GetReqUrl();
    string seq    = rtsp.GetCSeq();
    string sessid = rtsp.GetSession();
...
void
GWRTSPSession::onTeardown(stk::RtspMsg &rtsp)
{
    string url    = rtsp.GetReqUrl();
    string seq    = rtsp.GetCSeq();
    string sessid = rtsp.GetSession();
...
void
GWRTSPSession::onGetParameter(stk::RtspMsg &rtsp)
{
    string seq    = rtsp.GetCSeq();
    string sessid = rtsp.GetSession();
...
void
GWRTSPSession::onWrite(Bufferev *bev)
{

}

void
GWRTSPSession::onClose(Bufferev *bev)
{
