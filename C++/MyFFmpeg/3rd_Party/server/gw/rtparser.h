...
public:
    ...
    friend  void       h264_to_ps_cb(uint8_t *data, int size, void* userdata, int type, uint64_t pts);
...
protected:
    ...
    int                rtp2ps(const uint8_t *data, size_t size, bool &bMark);
    ...
    int                h264ToPs(const uint8_t *data, size_t size, int type, uint64_t pts);
protected:
    int                m_encode;
    int                m_type;
    uint64_t           m_pts;
