...
RtspClient::RtspClient(const char *ip, int port)
{	
    m_strIp = ip;
    m_rPort = port;
    m_s32ContentLen = 0;
...
bool RtspClient::startex(const char * strUrl, void *pUser)
{
    m_strUrl = strUrl;
    m_pUser = pUser;
...
size_t get_parameter_callback(const char *ptr, size_t size, size_t nmemb, void *user)
{
    RtspClient *rtsp = (RtspClient *)user;
    if (rtsp)
    {
        ...
        {
            stk::RtspMsg rtspMsg(rtsp->m_strResponse.c_str(), rtsp->m_strResponse.size());
            rtsp->onGetParameter(rtspMsg, rtsp->m_pUser);
        }
    }
...
bool RtspClient::doGetParameter()
{
    AutoLock lock(m_mtx);
    ...
    if (NULL != m_pCurl)
    {
        ...
        CURL_EASY_SETOPT(m_pCurl, CURLOPT_RTSP_STREAM_URI, m_strUrl.c_str());
        CURL_EASY_SETOPT(m_pCurl, CURLOPT_RTSP_REQUEST, (long)CURL_RTSPREQ_GET_PARAMETER);

        m_strResponse = "";
        m_s32ContentLen = 0;
        CURL_EASY_SETOPT(m_pCurl, CURLOPT_WRITEFUNCTION, get_parameter_callback);
        CURL_EASY_SETOPT(m_pCurl, CURLOPT_WRITEDATA, this)
        CURL_EASY_PERFORM(m_pCurl);
...
void RtspClient::onGetParameter(stk::RtspMsg &rsp, void *pUser)
{
    string strMsg;
    rsp.to_str(strMsg);
    printf("%s", strMsg.c_str());
